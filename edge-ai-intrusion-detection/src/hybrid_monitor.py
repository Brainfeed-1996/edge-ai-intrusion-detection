...
# Prometheus Metrics
INFERENCE_LATENCY = Histogram('edge_inference_latency_seconds', 'Latency of C++ engine inference')
THREATS_DETECTED = Counter('edge_threats_detected_total', 'Total threats detected by AI layer')
...
def monitor_edge():
    print("[INFO] Starting Edge Hybrid Monitor with Metrics on :8001...")
    start_http_server(8001)
    while True:
        # Simulate packet capture
        mock_packet = [random.randint(0, 255) for _ in range(1500)]
        
        # Call high-performance C++ engine with latency tracking
